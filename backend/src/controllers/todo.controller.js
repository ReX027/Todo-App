...
const fetchTodo = asynchandler(async (req, res) => {
  const todos = await Todo.find({ createdBy: req.user.id }).select(
    "_id createdBy content complete color"
  );
  if (!todos) {
...
const addTodo = asynchandler(async (req, res) => {
  ...
  return res
    .status(201)
    .json(new ApiResponse(200, createdTodo, "Todo is added successfully"));
});
